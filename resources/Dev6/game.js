...
function addWalls(x, y){
	if(mazeData[x][y+1] == 1)
		walls.push({x:x, y:y+1, dir:"north"}); // north
	if(mazeData[x][y-1] == 1)
		walls.push({x:x, y:y-1, dir:"south"}); // south
	if(mazeData[x+1][y] == 1)
		walls.push({x:x+1, y:y, dir:"east"}); // east
	if(mazeData[x-1][y] == 1)
		walls.push({x:x-1, y:y, dir:"west"}); // west
}
...
function generateMaze(width, height) {
	...
	while(walls.length != 0){
		...
		switch(wall.dir){
			case "north":
				afterCell = {x:wall.x, y:wall.y+1};
				break;
			case "south":
				afterCell = {x:wall.x, y:wall.y-1};
				break;
			case "east":
				afterCell = {x:wall.x+1, y:wall.y};
				break;
			case "west":
				afterCell = {x:wall.x-1, y:wall.y};
				break;
		...
		if(mazeData[afterCell.x][afterCell.y] == 1){ // after cell not visited/filled			
			// in the maze, wall is turned into a floor
			mazeData[wall.x][wall.y] = 0;
			// so is the aftercell
			mazeData[afterCell.x][afterCell.y] = 0;

			// add aftercell's walls to wall list
			addWalls(afterCell.x, afterCell.y);
		}
...
function spawnInRow(spawned, forcedX){
	// randomize pos
	var randY = 1;
	var checkLocation = PS.color(forcedX, randY);

	while(checkLocation != PS.COLOR_WHITE){
		// avoid borders and walls being on borders w/ -2
		var randY = getRandomInt(2, mazeSize - 3);
		checkLocation = PS.color(forcedX, randY);
	}

	// new pos
	PS.color(forcedX, randY, spawned);

	return {x:forcedX, y:randY};
}

function spawnInColumn(spawned, forcedY){
	// randomize pos
	var randX = 1; 
	var checkLocation = PS.color(randX, forcedY);

	while(checkLocation != PS.COLOR_WHITE){
		// avoid borders and walls being on borders w/ -2
		var randX = getRandomInt(2, mazeSize - 3); 
		checkLocation = PS.color(randX, forcedY);
	}

	// new pos
	PS.color(randX, forcedY, spawned);

	return {x:randX, y:forcedY};
}

function spawnAtOppositeBeads(player, goal){
	var rowOrCol = getRandomInt(0,1);
	var chance = getRandomInt(0,1);

	var dataReturn = {player:null, goal:null};

	if(rowOrCol == 0){ 
		if(chance == 0){ 
			dataReturn.player = spawnInRow(player, 2);
			dataReturn.goal = spawnInRow(goal, mazeSize-1 - 2);
		}
		else{ 
			dataReturn.player = spawnInRow(player, mazeSize-1 - 2);
			dataReturn.goal = spawnInRow(goal, 2);
		}
	}
	else {	
		if(chance == 0){ 
			dataReturn.player = spawnInColumn(player, 2);
			dataReturn.goal = spawnInColumn(goal, mazeSize-1 - 2);
		}
		else{ 
			dataReturn.player = spawnInColumn(player, mazeSize-1 - 2);
			dataReturn.goal = spawnInColumn(goal, 2);
		}
	}
...
PS.init = function( system, options ) {
	...
	PS.statusText("GET TO THE GOAL USING ARROW KEYS");
	...
	// add in key pieces
	var playerNGoal = spawnAtOppositeBeads(playerVisual, goalVisual);
	playerPos = playerNGoal.player;
	goalPos = playerNGoal.goal;
